...
// Configuration
const PROGRESS_FILE = "./data/cross-oracle-progress.json";
const MAX_RETRIES = 3;
...
async function loadProgress(): Promise<Progress> {
  try {
    if (existsSync(PROGRESS_FILE)) {
      ...
      console.log(
        `Found existing progress: ${progress.results.length} oracles analyzed`
      );
      return progress;
    }
  } catch (error) {
...
async function analyzeCrossOracle(
  ...
  try {
    const crossContract = new ethers.Contract(
      address,
      CROSS_ORACLE_ABI,
      provider
    );
    ...
    const baseCrossAddress =
      (await crossContract.oracleBaseCross?.()) as string;
    if (!baseCrossAddress) {
    ...
    const crossQuoteAddress =
      (await crossContract.oracleCrossQuote?.()) as string;
...
async function analyzeCrossOracles(): Promise<void> {
  ...
  try {
    const data = await fs.readFile(
      "/src/scrape-euler-adapters/output/euler-oracles.json",
      "utf-8"
    );
  ...
  const crossOracles = oracles
    ...
    .filter(
      (oracle): oracle is OracleData & { fullAddress: string } =>
        oracle.fullAddress !== null
    );
  ...
  if (crossOracles.length === 0) {
    console.log(
      "No Cross oracles found. Make sure the data has been scraped and analyzed."
    );
    process.exit(0);
  }
  ...
  const remainingOracles = crossOracles.filter(
    (oracle) => !completedAddresses.includes(oracle.fullAddress)
  );

  console.log(
    `Remaining oracles to analyze: ${remainingOracles.length}/${crossOracles.length}\n`
  );
  ...
  for (let i = 0; i < remainingOracles.length; i++) {
    ...
    console.log(
      `[${totalProcessed}/${crossOracles.length}] Processing ${address}`
    );
    ...
    try {
      ...
      if (result.error) {
        ...
        if (consecutiveFailures >= MAX_CONSECUTIVE_FAILURES) {
          ...
          console.error(
            "This might indicate persistent rate limiting or RPC issues."
          );
          console.error(
            "Please wait a few minutes and run the script again to continue.\n"
          );
          ...
          console.log(
            `Progress saved. ${completedAddresses.length}/${crossOracles.length} oracles completed.`
          );
          process.exit(1);
        }
      } else {
  ...
  await fs.writeFile(
    "data/cross-oracle-analysis.json",
    JSON.stringify(output, null, 2)
  );
  console.log("\n✓ Saved detailed results to data/cross-oracle-analysis.json");

  // Save CSV
  const csvHeader =
    "Cross Address,Base,Quote,Page,Base Cross Address,Base Cross Name,Cross Quote Address,Cross Quote Name,Error\n";
  ...
  Object.entries(nameCounts)
    ...
    .forEach(([name, count]) => {
      const percentage = (
        (count / (output.summary.successful * 2)) *
        100
      ).toFixed(1);
      console.log(
        `  ${name.padEnd(25)} ${count.toString().padStart(3)} (${percentage}%)`
      );
    });
  ...
  results
    ...
    .forEach((r) => {
      console.log(
        `  ${r.base}/${r.quote}: ${r.baseCrossName} + ${r.crossQuoteName}`
      );
    });
